...
- [x] `packages/cli/src/worker-host/native-terminal-launcher.ts`의 tmux attach 기본동작을 interactive TTY에서는 현재 터미널 attach로 바꾸고, 비대화형 환경에서만 Terminal.app fallback을 유지하도록 수정
- [x] `packages/cli/src/commands/start.ts`, `packages/cli/src/worker-bootstrap-context.ts`, `packages/cli/src/worker-control-plane-client.ts`, `packages/gateway/src/worker-registry.ts`를 정리해 워커 이름 수동 지정 옵션을 제거하고 기본 이름을 프로젝트 폴더명 + `-1`, `-2` 자동 넘버링 규칙으로 통일
- [x] `packages/web/src/App.tsx`, `packages/web/src/components/dashboard/WorkerLogPanel.tsx`를 정리해 워커 `terminal` 모달이 live output 버퍼 대신 authoritative worker projection snapshot을 우선 사용하고, 모달이 열린 동안 선택 워커 projection을 더 자주 새로고침해 tmux pane과 대시보드 터미널이 같은 화면을 보도록 동기화
- [x] `packages/web/src/App.tsx`의 active console surface에서 legacy run/session 분기(`PhaseProgress`, `TaskList`, `AgentStream`, `SessionOutputPanel`)를 제거해 새 authority-first dashboard surface만 렌더되도록 정리
- [x] `packages/web/src/hooks/useOlympus.ts`의 worker config 표시 이름을 avatar alias 대신 실제 registered worker name으로 바꿔 worker card, monitor, chat, terminal 표면이 새 자동 worker naming 규칙과 일치하도록 정리
- [x] `packages/web/src/hooks/useOlympus.ts`의 공개 return 경계를 새 dashboard read model 기준으로 좁히고, legacy run/session action(`subscribe*`, `cancel`, `connectAvailableSession`, `sendAgentCommand`)은 내부 compatibility state로만 남도록 정리
- [x] `packages/web/src/hooks/useOlympusLegacyRuntime.ts`를 추가해 `useOlympus`의 legacy run/session 초기 state와 websocket handler(`runs/sessions/snapshot/task/session*`)를 별도 compatibility helper로 분리
- [x] `packages/web/src/components/ProjectedTerminalPanel.tsx`, `packages/web/src/components/dashboard/WorkerLogPanel.tsx`, `packages/web/src/components/dashboard/WorkerCard.tsx`, `packages/web/src/components/monitor/OlympusTempleMonitor.tsx`의 terminal surface를 `projected console` 계약에 맞게 재표현하고, native tmux attach가 full terminal 경로임을 UI에 명시
- [x] `packages/web/src/components/*` 와 `packages/web/src/hooks/useContextTree.ts`에서 활성 dashboard 경로가 더 이상 참조하지 않는 legacy console/settings/browser surface(`AgentStream`, `TaskList`, `PhaseProgress`, `SessionOutputPanel`, `Header`, `SettingsModal`, `ProjectBrowser`, `GatewayEventLog` 등)를 삭제해 web 패키지의 혼합 모델 잔재를 정리
- [x] `packages/web/src/hooks/useOlympus.ts`에서 남아 있던 legacy run/session compatibility state(`phase/tasks/runs/sessions/sessionOutputs/sessionScreens`)와 관련 helper를 제거하고, 공개 return에서도 더 이상 쓰이지 않는 `cliHistory/cliStreams`를 제외해 active dashboard read model만 남도록 축소
- [x] `packages/web/src/hooks/useOlympus.ts`에서 worker behavior heuristic에만 쓰이던 `cliHistory/cliStreams/cliSessions` 내부 상태와 polling/stream 추적을 제거하고, worker behavior는 registry truth + 최근 dashboard state 기준으로 단순화
- [x] `packages/web/src/components/dashboard/CliSessionsPanel.tsx`를 삭제해 web 패키지에 남아 있던 마지막 unused legacy dashboard artifact를 제거
- [x] `packages/telegram-bot/src/index.ts`를 정리해 일반/직접 Telegram 메시지 경로를 `POST /api/codex/chat` + `worker:task:*` current control-plane 이벤트 기준으로 통일하고, old `api/cli/run*`/`cli:stream`/`session:*` transport 의존을 제거
- [x] `packages/telegram-bot/src/draft-stream.ts`와 stale telegram unit tests를 제거 또는 current control-plane 계약 기준으로 재작성해 telegram bot 패키지 테스트 경계가 더 이상 legacy CLI/session model을 전제하지 않도록 정리
