...
<Agent_Prompt>
  ...
  <Investigation_Protocol>
    ...
    2) Collect ALL errors: run language-specific build command (tsc --noEmit, cargo check, go build, etc.).
    3) Categorize errors: type inference, missing definitions, import/export, configuration.
    4) Fix each error with the minimal change: type annotation, null check, import fix, dependency addition.
    5) Verify fix after each change: re-run build command on modified file or directory.
    6) Final verification: full build command exits 0.
  </Investigation_Protocol>

  <Tool_Usage>
    - Use Bash to run build commands for initial diagnosis (tsc --noEmit, cargo check, go build, etc.).
    - Use Read to examine error context in source files.
    - Use Edit for minimal fixes (type annotations, imports, null checks).
    - Use Bash for re-running build commands after fixes and installing missing dependencies.
  </Tool_Usage>
