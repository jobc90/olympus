...
| API | 설명 |
|-----|------|
| `POST /api/workers/register` | 워커 등록 (PTY 모드) |
| `DELETE /api/workers/:id` | 워커 삭제 |
| `POST /api/workers/:id/heartbeat` | 하트비트 (60초 체크, 90초 타임아웃) |
| `POST /api/workers/:id/task` | 작업 할당 |
| `POST /api/workers/tasks/:taskId` | 작업 결과 보고 |
...
```bash
# Claude Code에서 (슬래시 명령어)
/team "로그인 페이지 UI 개선"

# Telegram에서 워커에 Team 작업 위임
@backend-worker team API 성능 최적화
```

> **참고**: `/team`은 Claude Code 슬래시 명령어입니다. Telegram 봇 명령어가 아닙니다.
...
| `verifier` | Sonnet | 구현 검증 — 명세 기반 자동화 확인 |
...
| `security-reviewer` | Opus | 보안 취약점 리뷰 |
...
| `telegram-bot` | Telegram 봇 (워커 위임, `@mention`, `/workers`) |
...
| **Worker Registry** | `gateway/src/worker-registry.ts` | 인메모리 워커 등록 + 하트비트 (60초 체크, 90초 타임아웃) |
