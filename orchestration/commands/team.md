...
> Shared workflow commands and skills are sourced from
> `https://github.com/jobc90/claudex-power-commands`.
> Claude should use `/check`, `/cowork`, `/super`, `/docs`, `/design`.
> Codex should use `$check`, `$cowork`, `$super`, `$docs`, `$design`.
> Do not rely on repo-local `orchestration/skills/` copies.
...
| **Step 5** | WI 완료 수신 → C-1 검증 → [AUTO] 다음 클레임 허용 / [DIRECTED] shutdown→새spawn → worktree WI는 `/check` 기반 git handoff 준비 | 리더가 파일/git 직접 수정 |
| **Step 6** | code-reviewer + style-reviewer 병렬 → 조건부 전문 리뷰어 | vision 없이 웹 QA 중복 실행 |
| **Step 8** | 웹앱→`webapp-testing`/Playwright, CLI→qa-tester Task | 증거 없이 PASS 판정 |
| **Step 9** | writer → `/check` 기반 git handoff → 팀원 shutdown 확인 → TeamDelete | TeamDelete 전 팀원 미종료 |
...
SendMessage → 리더에게 브랜치명 + 변경 요약 보고
    ...
    └─ 변경 있음 → 브랜치 보존
                   → 리더가 `/check --commit` handoff로 머지/정리 실행
```

**머지 처리 → `/check --commit` handoff로 처리**:
```
/check --commit
# 또는 Codex
Use $check --commit for the worktree merge handoff.
```

리더는 preserved worktree branch의 diff를 검토한 뒤 `/check --commit` 또는
`$check --commit`으로 최종 git 정리를 수행합니다.
...
3. 두 번째 워크트리 머지 시 충돌 발생:
   ...
   → 충돌 파일이 SHARED에 속하면
      → `/check` 기반 handoff로 충돌 해소:
        "worktree-{A}와 worktree-{B} 머지 중 {file} 충돌. SHARED 파일이므로
         양쪽 변경을 병합하여 충돌 해소 후 검증과 함께 정리."
...
| **designer** | Sonnet | ✅ | UI/UX 설계 + 구현 | Step 3→5 (Team) | `/design` 워크플로우와 정렬 |
...
| **vision** | Sonnet | ❌ | 시각적 검증 | Step 6 (Task, 조건부) | Playwright/webapp-testing 스크린샷 |
| **verifier** | Sonnet | ❌ | 스펙 충족 검증 | Step 7 (Task) | — |
| **qa-tester** | Sonnet | ❌ | 웹앱/CLI/서비스 테스트 | Step 8 (Task) | `webapp-testing`/Playwright 또는 CLI 캡처 |
...
# claudex-power-commands 명령 확인
for cmd in check cowork super docs design; do
  test -f ~/.claude/commands/${cmd}.md && echo "${cmd}: ✅" || echo "${cmd}: ❌"
done

# 브라우저 자동화는 Playwright MCP 또는 webapp-testing 사용
echo "browser qa: Playwright MCP / webapp-testing"
```
...
| 도메인 신호 | 활성화 대상 |
|------------|------------|
| React/Next.js/UI | `/design`, `vercel-react-best-practices`, `ui-ux-pro-max`, `designer` |
| CSS/디자인/컴포넌트 | **`/design`** 또는 `$design`, `ui-ux-pro-max`, `designer` |
| DB/PostgreSQL | `postgres-best-practices` |
| API/REST/GraphQL | `codex_analyze` MCP, `api-reviewer` |
| 테스트/단위 | `webapp-testing`, `test-engineer` |
| **웹앱 QA/버그 탐색** | **`webapp-testing`** 또는 Playwright MCP, `qa-tester` |
| Git/커밋 | `/check`, `$check`, `git-master` 에이전트 |
...
QA 단계    webapp-testing/qa-tester ~$0.05
━━━━━━━━━━━━━━━━━━━━━━━
예상 총비용: $0.80~1.20
예상 소요:   40~60분
━━━━━━━━━━━━━━━━━━━━━━━
...
|-----------|---------------|-----------|--------------|
| UI/컴포넌트 | `designer` | — | **`/design`** 워크플로우 — Task Description에 aesthetic direction 포함 |
| 백엔드/도메인/인프라 | `executor` | — | — |
| 테스트 | `test-engineer` | — | — |
| **웹앱 QA** | `qa-tester` | — | **`webapp-testing`** 또는 Playwright — URL만 있으면 브라우저 탐색 + 재현 증거 리포트 |
...
# 1. Playwright MCP 또는 webapp-testing으로 스크린샷 획득
→ 스크린샷 파일 경로 확보
...
```
> Playwright/webapp-testing 사용 불가 / dev server 없음 / 스크린샷 획득 불가 시 → vision 에이전트 건너뛰고 code-reviewer Stage 2에서 코드 레벨 UI 검토로 대체.
...
| 애플리케이션 유형 | QA 방식 | 도구 |
|-----------------|---------|------|
| **웹앱/브라우저 UI** | **`webapp-testing` 또는 Playwright MCP** | 브라우저 탐색, 스크린샷, 재현 증거 리포트 |
| CLI/서비스/API | `qa-tester` Task | Bash 출력 캡처 |
| 유닛/통합 테스트 | 직접 실행 | pnpm test |

### 웹앱 — `webapp-testing` 또는 Playwright 우선 사용

**웹 QA 도구 선택**:

- `webapp-testing`을 사용할 수 있으면 우선 사용
- 아니면 Playwright MCP 사용
- 둘 다 어려우면 `qa-tester` Task로 대체
...
- dev server 미실행 시 → 먼저 `pnpm dev &` 실행 후 port 대기 (30초 내 응답 대기):
  ...
  ```
- 출력: QA report / 스크린샷 / Playwright trace
  - 인터랙티브 버그: 재현 영상(`.webm`) + 단계별 스크린샷
  - 정적 버그 (오타/레이아웃): 단일 annotated 스크린샷
- 리포트 보존: `cp -r .team/qa-evidence .qa-evidence-{team-slug} 2>/dev/null || true`
- CRITICAL/HIGH 이슈 발견 시 → Step 5 복귀

> 웹 QA는 앱 소스코드가 아니라 실제 브라우저에서 관찰된 동작과 증거만 기준으로 판정합니다.
...
### 9-2. `/check` 기반 git handoff — 커밋 정리 (필요 시, 팀원 아님)

```
/check --commit
# 또는 Codex
Use $check --commit after verification passes.
```

여러 팀원의 산발적 변경 → `/check` 또는 `$check`로 검토·검증 후 atomic commit, 메시지 통일, 기존 컨벤션 준수.
...
> **⚠️ Step 9의 writer와 `/check` handoff는 team_name 없는 일반 실행** — TeamDelete 영향 없음. 완료 대기 후 TeamDelete 호출.
...
- [ ] Evidence-Based QA: 증거 기반 통과 (웹앱: webapp-testing/Playwright evidence 확인, CLI: `.team/qa-evidence.txt` 확인) `[M: 직접 테스트 결과로 대체]`
